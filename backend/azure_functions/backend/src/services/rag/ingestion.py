...
def extract_text_from_batch(pdf_bytes: bytes, page_range: str, config) -> str:
    """
    Extract text from a pre-sliced PDF batch using Azure Document Intelligence.
    
    This is a public wrapper around the internal _extract_all_pages function
    that handles client initialization and returns a single merged text string.
    
    IMPORTANT: The pdf_bytes should be a PRE-SLICED PDF containing only the pages
    you want to extract. The page_range parameter is used only for logging/tracking
    the original page numbers - Azure will process ALL pages in the provided PDF.
    
    Args:
        pdf_bytes: Raw PDF file content (bytes) - should be pre-sliced to target pages
        page_range: Page range string for logging/tracking (e.g., "1-2", "3-4")
                   This represents the original page numbers before slicing
    ...
    """
    logger.info(f"Extracting text from pre-sliced batch (original pages {page_range}, PDF size: {len(pdf_bytes):,} bytes)")
    ...
    try:
        ...
        # Extract all pages from the pre-sliced PDF
        # NOTE: We don't pass page_range to Azure because pdf_bytes is already sliced
        page_texts, pages_detected = _extract_all_pages(client, pdf_bytes)
        
        if not page_texts:
            logger.warning(f"No text extracted from batch (original pages {page_range})")
            return ""
        ...
        logger.info(
            f"Extracted {len(merged_text)} characters from batch (original pages {page_range}, "
        ...
        logger.error(f"Failed to extract text from batch (original pages {page_range}): {e}")
        raise AzureServiceError(f"Failed to extract text from batch: {str(e)}") from e


def _extract_all_pages(client: DocumentIntelligenceClient, file_content: bytes) -> tuple[list[str], int]:
    """
    Extract text from ALL pages in the provided PDF.
    
    This function does NOT accept a page_range parameter - it processes
    all pages in the provided PDF. If you need to extract specific pages,
    slice the PDF first using pdf_utils.slice_pdf_to_batch().
    
    Args:
        client: Document Intelligence client
        file_content: Raw file content (bytes) - should be pre-sliced if needed
    ...
    # Call Azure Document Intelligence WITHOUT pages parameter
    # This processes all pages in the provided PDF
    poller = client.begin_analyze_document(
        model_id="prebuilt-read",
        body=BytesIO(file_content)
        # NOTE: No 'pages' parameter - process all pages in the PDF
    )
...
def _extract_page_range(client: DocumentIntelligenceClient, file_content: bytes, page_range: str) -> tuple[list[str], int]:
    """
    DEPRECATED: Use _extract_all_pages() with pre-sliced PDFs instead.
    
    Extract text from a specific page range (kept for backward compatibility with _batch_extract_pages).
