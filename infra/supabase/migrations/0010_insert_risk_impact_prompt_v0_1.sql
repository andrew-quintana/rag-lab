-- Migration: Insert risk impact evaluator prompt into prompts table
-- This migration inserts the risk impact evaluation prompt template with:
-- - version: 0.1 (semantic versioning, no "v" prefix)
-- - name: risk_impact_evaluator
-- - live: true (mark as live/active version)

-- Insert risk impact evaluator prompt
INSERT INTO prompts (version, prompt_type, name, prompt_text, live)
VALUES (
    '0.1',
    'evaluation',
    'risk_impact_evaluator',
    $prompt$# System-Level Risk Impact Calculation Prompt
...
ON CONFLICT (prompt_type, name, version) DO UPDATE 
