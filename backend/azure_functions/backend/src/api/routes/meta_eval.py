...
for validating judge verdicts is available via src.services.evaluator.meta_eval.meta_evaluate_judge().
"""
...
async def handle_meta_eval(request: MetaEvalRequest):
    ...
    For meta-evaluation of judge verdicts, use src.services.evaluator.meta_eval.meta_evaluate_judge()
    ...
    raise HTTPException(
        status_code=501, 
        detail="Version comparison meta-evaluation endpoint not yet implemented. Use src.services.evaluator.meta_eval.meta_evaluate_judge() for judge verdict validation."
    )
